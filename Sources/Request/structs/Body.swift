...
extension Body {
    public init(data: [UInt8], file: String, name: String, type: String = "image/jpeg") {
        ...
        data.append(contentsOf: [UInt8]("Content-Disposition: form-data; name=\"\(name)\"; filename=\"\(file)\"\r\n".utf8) )
        data.append(contentsOf: [UInt8]("Content-Type: \(type)\r\n\r\n".utf8) )
        data.append(contentsOf: [UInt8]("\r\n--\(uuid)--\r\n".utf8) )
        self.init(data: data, boundary: uuid)
    }
}
